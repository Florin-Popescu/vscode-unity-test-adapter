...
`unityExplorer.unitUnderTestGlobPattern`| A glob pattern which should match all unit under test files. These are tracked for changes to mark test results as old if not re-run. By default the workspace root is used. | `src/**/*.{c,h}`
`unityExplorer.testSourceGlobPattern`   | A glob pattern which should match all unit test source files. By default the workspace root is used. | `testsrc/**/*Test.{c,h}`
